...
class PerceptualLoss(nn.Module):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.vgg_features = vgg.features[:30]  # 到 relu5_3
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            layer_weights = [1.5, 1.5, 2.0, 2.0, 1.5]
            perceptual_loss = 0.0
            for w, pred_feat, target_feat in zip(layer_weights, pred_features, target_features):
...
class EdgeLoss(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        return (
            torch.nn.functional.l1_loss(torch.diff(pred, dim=-1), torch.diff(target, dim=-1))
            + torch.nn.functional.l1_loss(torch.diff(pred, dim=-2), torch.diff(target, dim=-2))
        )

# Import discriminator builder
try:
    from src.npu.networks.discriminator import build_discriminator
except ImportError:
    import sys
...
try:
    from train.gan_losses import build_gan_loss
except ImportError:
    import sys
    sys.path.append(os.path.join(os.path.dirname(__file__), '..'))
    from gan_losses import build_gan_loss
...
try:
    ...
    try:
        ...
        class PatchTensorBoardLogger:
            ...
            def log_validation_step(self, *args, **kwargs):
                pass
            def finalize_validation_epoch(self, *args, **kwargs):
                pass
...
class PatchAwareLoss(nn.Module):
    ...
    def __init__(self, 
        ...
        # NEW: mu-law parameter for tone-mapping
        self.mulaw_mu = float(self.hdr_vis_cfg.get('mulaw_mu', 500.0))
        ...
        # NEW: Loss domain control (default LDR for perceptual/boundary; pixel/edge configurable)
        self.loss_domain = str((weights_config or {}).get('domain', 'ldr')).lower()
        self.ldr_for_pixel = bool((weights_config or {}).get('ldr_for_pixel', self.loss_domain == 'ldr'))
        self.ldr_for_edge = bool((weights_config or {}).get('ldr_for_edge', self.loss_domain == 'ldr'))
    ...
    def forward(self, 
        ...
        if 'patch' in predictions:
            ...
            # 基础patch损失（像素重建项，支持鲁棒损失）
            if getattr(self, 'ldr_for_pixel', False):
                ldr_pred = self._tone_map_for_vgg(patch_pred)
                ldr_tgt  = self._tone_map_for_vgg(patch_target)
                patch_l1 = self._robust_reconstruction_loss(ldr_pred, ldr_tgt)
            else:
            ...
            # 边缘损失：在 HDR 或 μ-law LDR 上
            if getattr(self, 'ldr_for_edge', False):
                ldr_pred_e = self._tone_map_for_vgg(patch_pred)
                ldr_tgt_e  = self._tone_map_for_vgg(patch_target)
                if self.mask_edge and mask_weight is not None:
                    mask3e = mask_weight.expand_as(ldr_pred_e)
                    pred_edge_m = ldr_pred_e * mask3e + ldr_tgt_e * (1.0 - mask3e)
                else:
                    pred_edge_m = ldr_pred_e
                patch_edge = self.edge_loss(pred_edge_m, ldr_tgt_e)
            else:
                if self.mask_edge and mask_weight is not None:
            ...
            if metadata and isinstance(metadata, dict) and 'holes_mask' in metadata:
                ...
                if self.w_hole > 0.0:
                    if getattr(self, 'ldr_for_pixel', False):
                        ldr_pred_h = self._tone_map_for_vgg(patch_pred)
                        ldr_tgt_h  = self._tone_map_for_vgg(patch_target)
                        hole_l1 = self._robust_masked_loss(ldr_pred_h, ldr_tgt_h, hole_weight)
                    else:
                ...
                if self.w_ctx > 0.0:
                    ...
                    if getattr(self, 'ldr_for_pixel', False):
                        ldr_pred_c = self._tone_map_for_vgg(patch_pred)
                        ldr_tgt_c  = self._tone_map_for_vgg(patch_target)
                        ctx_preserve = self._robust_masked_loss(ldr_pred_c, ldr_tgt_c, ctx_mask)
                    else:
            ...
            # NEW: 局部感知（patchs-in-a-patch）：在洞内显著网格单元上计算VGG并按洞面积加权
            local_perc_loss = torch.tensor(0.0, device=patch_pred.device)
            if self.local_perc_enable and self.w_local_perc > 0.0 and (metadata and isinstance(metadata, dict) and 'holes_mask' in metadata):
                holes_mask_lp = torch.clamp(metadata['holes_mask'], 0.0, 1.0)
                tm_pred = self._tone_map_for_vgg(patch_pred)
                tm_tgt  = self._tone_map_for_vgg(patch_target)
                if holes_mask_lp.shape[2:] != tm_pred.shape[2:]:
                    holes_mask_lp = F.interpolate(holes_mask_lp, size=tm_pred.shape[2:], mode='nearest')
                N, _, H, W = tm_pred.shape
                gh, gw = max(1, H // self.local_perc_grid_rows), max(1, W // self.local_perc_grid_cols)
                num_acc = None
                den_acc = 0.0
                for i in range(N):
                    hm = holes_mask_lp[i, 0]
                    pred_i = tm_pred[i:i+1]
                    tgt_i  = tm_tgt[i:i+1]
                    selected = 0
                    for r in range(self.local_perc_grid_rows):
                        y1 = r * gh; y2 = H if r == self.local_perc_grid_rows - 1 else (r+1) * gh
                        for c in range(self.local_perc_grid_cols):
                            x1 = c * gw; x2 = W if c == self.local_perc_grid_cols - 1 else (c+1) * gw
                            cell = hm[y1:y2, x1:x2]
                            if cell.numel() == 0:
                                continue
                            frac = float(cell.mean().item())
                            if frac < self.local_perc_hole_thresh:
                                continue
                            pad = self.local_perc_padding
                            yy1 = max(0, y1 - pad); yy2 = min(H, y2 + pad)
                            xx1 = max(0, x1 - pad); xx2 = min(W, x2 + pad)
                            lp_loss = self.perceptual_loss(pred_i[:, :, yy1:yy2, xx1:xx2], tgt_i[:, :, yy1:yy2, xx1:xx2])
                            area_i = float(cell.sum().item())
                            if area_i > 0.0:
                                num_acc = (num_acc + area_i * lp_loss) if num_acc is not None else (area_i * lp_loss)
                                den_acc += area_i
                            selected += 1
                            if selected >= self.local_perc_max_crops:
                                break
                        if selected >= self.local_perc_max_crops:
                            break
...
class PatchFrameInterpolationTrainer(pl.LightningModule):
    ...
    def __init__(self,
        ...
        # 创建PatchNetwork
        inpainting_config = model_config.get('inpainting_network', model_config)
        ...
        # GAN / adversarial training configuration（默认禁用）
        gan_cfg = (full_config or {}).get('gan', {}) if full_config else {}
        self.gan_config = gan_cfg
        self.gan_enabled = bool(gan_cfg.get('enable', False))
        self.gan_warmup_epochs = int(gan_cfg.get('warmup_epochs', 0))
        self.lambda_adv_start = float(gan_cfg.get('lambda_adv_start', 0.0))
        self.lambda_adv_end = float(gan_cfg.get('lambda_adv_end', self.lambda_adv_start))
        self.lambda_adv_warmup_epochs = int(gan_cfg.get('lambda_adv_warmup_epochs', 0))
        self.gan_d_steps = int(gan_cfg.get('training', {}).get('d_steps', 1)) if isinstance(gan_cfg.get('training', {}), dict) else 1
        self.gan_g_steps = int(gan_cfg.get('training', {}).get('g_steps', 1)) if isinstance(gan_cfg.get('training', {}), dict) else 1
        self.discriminator = None
        self.adversarial_loss = None
        self.gan_step_enabled = False
        self.current_lambda_adv = 0.0
        self._warmup_scheduler_ref = None
        self._cosine_scheduler_ref = None
        if self.gan_enabled:
            self.discriminator = build_discriminator(self.gan_config)
            self.adversarial_loss = build_gan_loss(self.gan_config)
            self.automatic_optimization = False
            print("GAN adversarial training enabled.")
        else:
        ...
        # Patch专用可视化系统
        vis_cfg_raw = full_config.get('visualization', {}) if full_config else {}
        viz_config = {
            'visualization_frequency': vis_cfg_raw.get('visualization_frequency', 200),
            'save_frequency': vis_cfg_raw.get('save_frequency', 500),
            'enable_visualization': vis_cfg_raw.get('enable_visualization', True),
            'hdr_processing': full_config.get('hdr_processing', {}),
            'data_root': full_config.get('data', {}).get('data_root', './data'),
            'validation_visualization_interval': vis_cfg_raw.get('validation_visualization_interval', 10)
        }
        self.val_visualization_interval = max(1, int(viz_config.get('validation_visualization_interval', 10)))
        ...
        # Log-domain dithering (optional)
        self.log_dither_enable = bool(norm_unified.get('log_dither_enable', False))
        self.log_dither_scale = float(norm_unified.get('log_dither_scale', 1.0e-6))
        print(f"Normalization mode: {self.norm_type}")

        # Sync student patch inpainting config with training normalization settings
        # 训练统计
        self.training_stats = {
            'patch_steps': 0,
            'recent_losses': [],
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """前向传播"""
        return self.patch_network(x)

    def _tone_map_for_vgg(self, x: torch.Tensor) -> torch.Tensor:
        mapper = getattr(self.patch_loss, "_tone_map_for_vgg", None)
        if callable(mapper):
            return mapper(x)
        return x
    ...
    def _apply_loss_weight_schedule(self, step: int) -> None:
        ...
        try:
            self.log('sched_w_perc', w_perc, on_step=False, on_epoch=True, prog_bar=False)
            self.log('sched_w_edge', w_edge, on_step=False, on_epoch=True, prog_bar=False)
            self.log('sched_w_boundary', w_boundary, on_step=False, on_epoch=True, prog_bar=False)
            self.log('sched_w_hole', w_hole, on_step=False, on_epoch=True, prog_bar=False)
            self.log('sched_w_ctx', w_ctx, on_step=False, on_epoch=True, prog_bar=False)
            self.log('sched_w_log', w_log, on_step=False, on_epoch=True, prog_bar=False)
        except Exception:
            pass
    ...
    def training_step(self, batch: Dict[str, torch.Tensor], batch_idx: int) -> Dict[str, torch.Tensor]:
        ...
        if 'patch_input' in batch and len(batch['patch_input']) > 0:
            ...
            if self.norm_type == 'global':
                ...
                if self.log_dither_enable:
                    dither = (torch.rand_like(warped_pos) - 0.5) * self.log_dither_scale
                    warped_pos = torch.clamp(warped_pos + dither, min=0.0)
        ...
        recon_loss, loss_dict = self.patch_loss(
        ...
        # 对抗训练相关变量初始化
        loss_adv = torch.tensor(0.0, device=recon_loss.device)
        loss_d = torch.tensor(0.0, device=recon_loss.device)
        total_loss = recon_loss

        if self.gan_enabled and self.discriminator is not None:
            opt_g, opt_d = self.optimizers()
            hole_mask = batch['patch_input'][:, 3:4]
            ...
            opt_g.step()
            if self._warmup_scheduler_ref is not None:
                self._warmup_scheduler_ref.step()

            self.training_stats['recent_losses'].append(total_loss.detach().item())
        else:
            # 自动优化路径（无GAN）
            total_loss = recon_loss
            self.training_stats['recent_losses'].append(total_loss.item())
        ...
        # 日志记录
        if self.gan_enabled:
            loss_dict['adv'] = loss_adv.detach()
        log_entries = {
            'train_loss': total_loss.detach() if self.gan_enabled else total_loss,
            'train_recon_loss': recon_loss.detach() if self.gan_enabled else recon_loss,
            'patch_probability': patch_probability,
            'mode': 0,
            **{f'train_{k}': v for k, v in loss_dict.items() if k != 'total' and v is not None}
        }
        if self.gan_enabled:
            log_entries['train_adv_loss'] = loss_adv.detach()
        ...
        try:
            ...
            self.log('train_hole_frac_mean', frac, on_step=False, on_epoch=True, prog_bar=False)
            self.log('train_hole_nonzero_ratio', nz, on_step=False, on_epoch=True, prog_bar=False)
        except Exception:
            pass
        
        # Patch可视化记录（改为按训练批次计数触发）
        current_batch_step = int(self.batch_step_count)
        if self.patch_visualizer and self.patch_visualizer.should_visualize(current_batch_step):
            try:
        ...
        # 兼容旧逻辑：保留但不再使用 visualization_step
        
        loss_to_return = total_loss if not self.gan_enabled else total_loss.detach()
    ...
    def validation_step(self, batch: Dict[str, torch.Tensor], batch_idx: int) -> Dict[str, torch.Tensor]:
        ...
        if 'patch_input' in batch and len(batch['patch_input']) > 0:
            ...
            if self.norm_type == 'global':
                ...
                patch_input_norm = patch_input.clone()
                patch_input_norm[:, :3] = Xn
                residual_pred_norm = self.patch_network(patch_input_norm, boundary_override=boundary_override) * scale
            elif self.norm_type == 'per_patch':
                warped_rgb = patch_input[:, :3]
                B = warped_rgb.shape[0]
                with torch.no_grad():
            ...
            if self.gan_enabled and self.discriminator is not None:
                with torch.no_grad():
                    ...
                    self.log('val_disc_fake', d_fake_val.mean().detach(), on_step=False, on_epoch=True, prog_bar=False)
        ...
        # 将验证损失写入Patch可视化日志，便于TensorBoard展示Validation分组
        if self.patch_visualizer:
            try:
                val_scalars = {'val_loss': val_loss}
                for k, v in val_loss_dict.items():
                    if k == 'total' or v is None:
                        continue
                    val_scalars[f'val_{k}'] = v
                self.patch_visualizer.log_validation_step(
                    epoch=self.current_epoch,
                    val_loss_dict=val_scalars
                )
            except Exception as e:
                if os.environ.get('PATCH_VIS_WARN', '0') == '1':
                    print(f"[WARN] Patch visualizer validation logging failed: {e}")
        ...
        if self.patch_visualizer and self.current_epoch % 5 == 0 and self.val_visualization_interval > 0:
            if batch_idx % self.val_visualization_interval == 0:
                try:
                    if (
                        'patch' in predictions and 'patch' in targets and
                        isinstance(predictions['patch'], torch.Tensor) and
                        ...
                        try:
                            n = int(predictions['patch'].shape[0])
                            k = min(4, n)
                            patch_inputs_vis = batch['patch_input'][:k]
                            patch_targets_vis = batch['patch_target_rgb'][:k]
                            patch_predictions_vis = predictions['patch'][:k]
                        except Exception:
                            patch_inputs_vis = batch['patch_input'][:1]
                            patch_targets_vis = batch['patch_target_rgb'][:1]
                            patch_predictions_vis = predictions['patch'][:1]

                        self.patch_visualizer.log_patch_comparison(
                            step=self.current_epoch * 1000 + max(0, batch_idx),
                            patch_inputs=patch_inputs_vis,
                            patch_targets=patch_targets_vis,
                            patch_predictions=patch_predictions_vis,
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        if self.patch_visualizer:
            try:
                self.patch_visualizer.finalize_validation_epoch()
            except Exception as e:
                if os.environ.get('PATCH_VIS_WARN', '0') == '1':
                    print(f"[WARN] Patch visualizer finalize failed: {e}")
    ...
    def on_after_backward(self) -> None:
        ...
        thresh = self.grad_clip_val if self.grad_clip_val is not None else float('inf')
        clipped = 1.0 if global_norm > thresh else 0.0

        # 记录到TensorBoard（Lightning的 self.log）
        self.log('grad_norm', global_norm, on_step=False, on_epoch=True, prog_bar=False)
        self.log('grad_norm_ema', self.grad_norm_ema, on_step=False, on_epoch=True, prog_bar=False)
        if self.grad_clip_val is not None:
            self.log('grad_clip_val', self.grad_clip_val, on_step=False, on_epoch=True, prog_bar=False)
        self.log('grad_clipped', clipped, on_step=False, on_epoch=True, prog_bar=False)
    ...
    def _log_recon_error_percentiles(self, pred: torch.Tensor, target: torch.Tensor, prefix: str) -> None:
        ...
        try:
            ...
            mean_err = err.mean()
            self.log(f'{prefix}_err_p50', p50, on_step=False, on_epoch=True, prog_bar=False)
            self.log(f'{prefix}_err_p90', p90, on_step=False, on_epoch=True, prog_bar=False)
            self.log(f'{prefix}_err_p95', p95, on_step=False, on_epoch=True, prog_bar=False)
            self.log(f'{prefix}_err_mean', mean_err, on_step=False, on_epoch=True, prog_bar=False)
        except Exception:
    ...
    def configure_optimizers(self) -> Dict[str, Any]:
        ...
        schedulers = []
        # step 级 warmup
        if (not self.gan_enabled) and warmup_scheduler is not None:
            schedulers.append({
        ...
        if not self.gan_enabled:
            ...
            self._warmup_scheduler_ref = warmup_scheduler
            self._cosine_scheduler_ref = scheduler
        if self.gan_enabled and self.discriminator is not None:
            disc_opt_cfg = self.gan_config.get('optimizer', {}) if isinstance(self.gan_config, dict) else {}
            lr_d = float(disc_opt_cfg.get('lr', 2e-4))
            betas = disc_opt_cfg.get('betas', [0.5, 0.999])
    ...
    def on_train_epoch_start(self) -> None:
        ...
        if self.gan_enabled:
            self.gan_step_enabled = self.current_epoch >= self.gan_warmup_epochs
            if self.gan_step_enabled:
                if self.lambda_adv_warmup_epochs > 0:
                    progress = (self.current_epoch - self.gan_warmup_epochs) / max(1, self.lambda_adv_warmup_epochs)
                    progress = max(0.0, min(1.0, progress))
                    self.current_lambda_adv = self.lambda_adv_start + (self.lambda_adv_end - self.lambda_adv_start) * progress
                else:
                    self.current_lambda_adv = self.lambda_adv_end
            else:
                self.current_lambda_adv = 0.0
        else:
            self.gan_step_enabled = False
    ...
    def on_epoch_end(self) -> None:
        ...
        if self.gan_enabled and self._cosine_scheduler_ref is not None:
            self._cosine_scheduler_ref.step()
    ...
    def _print_model_architecture_info(self, inpainting_config: Dict[str, Any]) -> None:
        ...
        patch_network = self.patch_network
        patch_network_channels = getattr(patch_network, 'ch1', base_channels)
        ...
        if base_channels != patch_network_channels:
            print(f"⚠️  警告: 配置的 base_channels ({base_channels}) 与实际网络通道 ({patch_network_channels}) 不一致，请检查YAML设置")
        
        print("💡 提示: 若需与推理脚本保持一致，请确保配置中的 base_channels 与推理侧设置相同")
...
def test_patch_training_framework():
    ...
    try:
        ...
        print(f"Patch网络参数: {sum(p.numel() for p in trainer.patch_network.parameters()):,}")
    ...
    try:
        ...
        model_config = {
            'inpainting_network': {
                ...
                'residual_scale_factor': network_config.get('residual_scale_factor', 1.0)
            }
        }
        ...
        trainer_config = {
            ...
            'gradient_clip_val': training_config.get('gradient_clip_val', 0.5 if not config.get('gan', {}).get('enable', False) else None),
            'accumulate_grad_batches': 1 if config.get('gan', {}).get('enable', False) else training_config.get('accumulate_grad_batches', 1),
        }
        ...
        # Inject overlapping-crop fields into PatchTrainingConfig instance (dataclass may not define them)
        for k in ['use_overlapping_crops', 'crop_size', 'crop_stride', 'keep_top_frac', 'min_hole_frac',
                  'sample_hole_weighted', 'enforce_min_hole_frac']:
            if k in patch_config_dict:
                setattr(patch_training_config, k, patch_config_dict.get(k))
        ...
        print(" Patch训练器创建成功")
        print(f" 网络参数: {sum(p.numel() for p in trainer.patch_network.parameters()):,}")
        ...
        if gc_val is not None:
            trainer_kwargs['gradient_clip_val'] = gc_val
        ...
        print(f"   梯度累计: {accum_batches} batches/step")
...
class ColleaguePatchDataset(Dataset):
    ...
    def __init__(self, data_root: str, split: str, patch_config, adapter_kwargs: Optional[Dict[str, Any]] = None):
        ...
        # Overlapping crops configuration (optional alternative to simple grid)
        self._use_overlap = bool(getattr(self.patch_config, 'use_overlapping_crops', False))
        self._crop_size = int(getattr(self.patch_config, 'crop_size', 256))
        self._crop_stride = int(getattr(self.patch_config, 'crop_stride', 128))
        self._keep_top_frac = float(getattr(self.patch_config, 'keep_top_frac', 0.5))
        self._min_hole_frac = float(getattr(self.patch_config, 'min_hole_frac', 0.0))
        self._overlap_crops = []

        # hole-aware sampling controls (simple grid)
        ...
        if self._use_overlap:
            print(" 初始化Overlapping Crop索引（按洞面积排序）")
            self._build_overlapping_crops_index()
            self._using_simple_grid = False
        elif patch_config.use_simple_grid_patches:
    ...
    def __len__(self):
        if self._use_overlap:
            return len(self._overlap_crops)
        return len(self.base_dataset) * self.patch_config.max_patches_per_image
    
    def __getitem__(self, idx):
        # 重叠crop模式
        if self._use_overlap:
            rec = self._overlap_crops[idx]
            image_idx = rec['image_idx']
            x1, y1 = rec['x'], rec['y']
            P = rec['w']
            try:
                input_tensor, target_residual, target_rgb = self.base_dataset[image_idx]
                patch_input = input_tensor[:, y1:y1+P, x1:x1+P]
                patch_target_residual = target_residual[:, y1:y1+P, x1:x1+P]
                patch_target_rgb = target_rgb[:, y1:y1+P, x1:x1+P]
                if not torch.is_tensor(patch_input):
                ...
                return {
                    ...
                    'metadata': {
                        ...
                        'crop_rect': (x1, y1, P, P),
                        'hole_frac': rec.get('hole_frac', 0.0)
                    }
                }
            except Exception as e:
                print(f" Overlap crop获取失败 [global_idx={idx}, image_idx={image_idx}]: {e}")
                P = self._crop_size
                return {
                    'patch_input': torch.zeros(7, P, P),
                    'patch_target_residual': torch.zeros(3, P, P),
                    'patch_target_rgb': torch.zeros(3, P, P)
                }
        ...
        try:
            ...
            #  放宽尺寸断言：仅断言通道
            assert patch_input.shape[0] == 7
            assert patch_target_residual.shape[0] == 3
            assert patch_target_rgb.shape[0] == 3
    ...
    def _build_overlapping_crops_index(self) -> None:
        keep_frac = max(0.0, min(1.0, self._keep_top_frac))
        total_frames = len(self.base_dataset)
        top_total = 0
        for img_idx in range(total_frames):
            try:
                input_tensor, _, _ = self.base_dataset[img_idx]
                ...
                P = min(self._crop_size, H, W)
                sx = self._crop_stride; sy = sx
                xs = list(range(0, max(1, W - P + 1), sx))
                ys = list(range(0, max(1, H - P + 1), sy))
                if len(xs) == 0 or xs[-1] != (W - P): xs = xs + ([W - P] if (W - P) >= 0 else [])
                if len(ys) == 0 or ys[-1] != (H - P): ys = ys + ([H - P] if (H - P) >= 0 else [])
                candidates = []
                for y in ys:
                    for x in xs:
                        cell = holes[y:y+P, x:x+P]
                        if cell.numel() == 0: continue
                        frac = float(cell.mean().item())
                        if self._min_hole_frac > 0.0 and frac < self._min_hole_frac: continue
                        candidates.append({'image_idx': img_idx, 'x': int(x), 'y': int(y), 'w': int(P), 'h': int(P), 'hole_frac': frac})
                if not candidates:
                    cx = max(0, (W - P) // 2); cy = max(0, (H - P) // 2)
                    frac = float(holes[cy:cy+P, cx:cx+P].mean().item())
                    candidates.append({'image_idx': img_idx, 'x': int(cx), 'y': int(cy), 'w': int(P), 'h': int(P), 'hole_frac': frac})
                candidates.sort(key=lambda r: r['hole_frac'], reverse=True)
                k = max(1, int(len(candidates) * keep_frac))
                self._overlap_crops.extend(candidates[:k])
                top_total += k
            except Exception as e:
                print(f"[WARN] 构建overlap crops失败 frame={img_idx}: {e}")
                continue
        print(f" Overlapping crops: frames={total_frames}, total_selected={top_total}, crop_size={self._crop_size}, stride={self._crop_stride}, keep_top_frac={keep_frac}")

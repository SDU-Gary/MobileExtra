...
class HierarchicalSpatialAttention(nn.Module):
    ...
    def __init__(self, channels: int, scales: list = [32, 16, 8]):
        ...
        for scale in scales:
            scale_attention = nn.Sequential(
                nn.AdaptiveAvgPool2d(scale),  # 下采样到指定尺度
        ...
        # 多尺度特征融合
        self.scale_fusion = nn.Conv2d(channels * self.num_scales, channels, kernel_size=1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 计算每个尺度的注意力
        for i, scale_attention in enumerate(self.scale_attentions):
            # 计算注意力权重
            att_weight = scale_attention(x)  # [B, C, scale, scale]
            
            # 上采样到原始尺寸
            att_weight = F.interpolate(att_weight, size=(H, W), 
                                     mode='bilinear', align_corners=False)
            
            # 应用注意力
            scale_feat = x * att_weight
            scale_features.append(scale_feat)
        
        # 加权融合多尺度特征
        weights = F.softmax(self.scale_weights, dim=0)
        fused_features = sum(weight * feat for weight, feat in zip(weights, scale_features))

...
def tone_map_mulaw(hdr: torch.Tensor, mu: float = 500.0, gamma: float = 1.0, exposure: float = 1.0,
                   adaptive_exposure: Optional[dict] = None) -> torch.Tensor:
    """Apply mu-law tone mapping then optional gamma correction.

    y = log(1 + mu * (x * exposure)) / log(1 + mu)
    with optional adaptive exposure pre-scaling and gamma correction.
    """
    x = _ensure_chw(hdr)
    exp = float(exposure) * _compute_adaptive_exposure(x, adaptive_exposure)
    if exp != 1.0:
    ...
    denom = torch.log(torch.tensor(1.0 + mu, dtype=x.dtype, device=x.device))
    ldr = torch.log(1.0 + mu * torch.clamp(x, min=0.0)) / denom
    if gamma != 1.0:
        ldr = torch.pow(torch.clamp(ldr, 1e-8, 1.0), 1.0 / gamma)
    return torch.clamp(ldr, 0.0, 1.0)
...
def tone_map(hdr: torch.Tensor, method: str = "reinhard", gamma: float = 2.2,
             exposure: float = 1.0, adaptive_exposure: Optional[dict] = None,
             mu: float = 500.0) -> torch.Tensor:
    ...
    if m == "mulaw":
        return tone_map_mulaw(hdr, mu=mu, gamma=gamma, exposure=exposure, adaptive_exposure=adaptive_exposure)
